...
public class Test 
{
			...
			throws Exception
	{
		...
		for(testDataRow = 1; testDataRow <= arr_testdata.length; testDataRow++)
		{
			...
			//执行测试步骤扫描
			stepThrough(filePath, sheetName);
		}	
		return result;
	}
	
	public void stepThrough(String filePath, String sheetName) throws Exception
	{
		f = new File(filePath);
		fis = new FileInputStream(f);
		wb = new XSSFWorkbook(fis);
    	...
    	for (int j = 1; j <= testSteps; j++) 
		{
            ...
            {
            	System.out.println("正在执行" + keyword + ", 正在点击" 
            						+ row.getCell(2).toString());
            ...
            else if(keyword.equals("Verify Element Text"))
            {
	            ...
	            System.out.println("正在执行" + keyword + ", 正在验证" 
	            				+ row.getCell(2).toString() + "的字符串是否与期望结果相等： "
	            				+ b);
	            if(!b)
	            {
	            	result = 0;
            ...
            else if(keyword.equals("Verify Element Display"))
            {
        		b = p.verifyElementDisplay(row.getCell(1).toString(), 
        				row.getCell(2).toString());
        		System.out.println("正在执行" + keyword + ", 正在验证元素" 
        						   + row.getCell(2).toString() 
        				+ "是否显示： " + b);
        		if(!b)
            	{
            ...
            else
            {
            	...
            	p.getPageObjects(System.getProperty("user.dir") 
            					+ "//src//com//testalliance//hrsystem" + "//objrepository//" 
            					+ module + "Page.properties");
        		stepThrough(System.getProperty("user.dir") + "//testdata.functions//" 
            					+ module + ".xlsx", keyword);
            }
		}
		wb.close();
